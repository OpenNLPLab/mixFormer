...
class RelativeMultiheadAttentionSuper(nn.Module):
    ...
    def __init__(self, super_embed_dim, num_heads, is_encoder, super_kdim=None, super_vdim=None, dropout=0., bias=True,
        ...
        if not self.k_only:
            self.relative_position_values = Parameter(
                torch.Tensor(2 * self.max_relative_length + 1, self.super_q_embed_dim))
    ...
    def set_sample_config(self, sample_embed_dim, sample_q_embed_dim, sample_attention_heads, sample_kv_embed_dim=None):
        ...
        self.qkv_dim = sample_q_embed_dim
        ...
        if not self.fixed:
            self.out_proj.set_sample_config(sample_in_dim=self.sample_q_embed_dim, sample_out_dim=self.sample_embed_dim)
