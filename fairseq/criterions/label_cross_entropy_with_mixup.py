...
class LabelSmoothedCrossEntropyMixupCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss, acc1, acc5 = self.compute_loss(model, net_output, sample, reduce=reduce)
    ...
    def aggregate_logging_outputs(logging_outputs):
        ...
        acc1 = sum(log.get('acc1', 0)*log.get('nsentences', 0) for log in logging_outputs)
        acc5 = sum(log.get('acc5', 0) * log.get('nsentences', 0) for log in logging_outputs)
        return {
            'loss': sum(log.get('loss', 0) for log in logging_outputs) / sample_size / math.log(2) if sample_size > 0 else 0.,
            'nll_loss': sum(log.get('nll_loss', 0) for log in logging_outputs) / ntokens / math.log(2) if ntokens > 0 else 0.,
            'acc1': acc1 / nsentences if nsentences > 0 else 0.,
            'acc5': acc5 / nsentences if nsentences > 0 else 0.,
            'ntokens': ntokens,
