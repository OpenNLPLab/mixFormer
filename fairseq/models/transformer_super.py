...
from fairseq.modules import (
    ...
    EmbeddingSuper,
    LinearSuper,
    LayerNormSuper, RelativeMultiheadAttention, LayerNorm, Linear
    ...
    def build_model(cls, args, task):
        ...
        if fixed:
            config = {
                ...
                'decoder': {
                    'layer': args.decoder_layer_num_subtransformer,
                    'self_attn_heads': args.decoder_self_attention_heads_all_subtransformer,
                    'en_de_attn_heads': args.decoder_ende_attention_heads_all_subtransformer,
                    'link': args.decoder_arbitrary_ende_attn_all_subtransformer,
                    'emb_dim': args.decoder_embed_dim_subtransformer,
                    'ffn_emb_dim': args.decoder_ffn_embed_dim_all_subtransformer
                }
            }

            args.encoder_embed_dim = args.encoder_embed_dim_subtransformer
            args.decoder_embed_dim = args.decoder_embed_dim_subtransformer
            if args.share_all_embeddings:
                max_emb_dim = min(
                    config['encoder']['emb_dim'], config['decoder']['emb_dim'])
                config['encoder']['emb_dim'] = max_emb_dim
                config['decoder']['emb_dim'] = max_emb_dim
                args.qkv_dim = max_emb_dim
                args.encoder_embed_dim = max_emb_dim
                args.decoder_embed_dim = max_emb_dim
                args.decoder_output_dim = max_emb_dim
        ...
        encoder = cls.build_encoder(
            args, src_dict, encoder_embed_tokens, config)
        decoder = cls.build_decoder(
            args, tgt_dict, decoder_embed_tokens, config)
        return TransformerSuperModel(encoder, decoder)

    @classmethod
    def build_encoder(cls, args, src_dict, embed_tokens, config):
        return TransformerEncoder(args, src_dict, embed_tokens, config=config)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens, config):
        return TransformerDecoder(args, tgt_dict, embed_tokens, config=config)
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, config=None):
        ...
        self.layers.extend([
            TransformerEncoderLayer(args, layer_idx=i, config=config)
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if self.fixed:
            x = self.sample_embed_scale * self.embed_tokens(src_tokens)
        else:
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, config=None):
        super().__init__(dictionary)
        ...
        if config is None:
            ...
            self.super_ffn_embed_dim = config['decoder']['ffn_emb_dim']
            self.super_layer_num = config['decoder']['layer']
            self.super_self_attention_heads = config['decoder']['self_attn_heads']
            self.super_ende_attention_heads = config['decoder']['en_de_attn_heads']
            self.super_arbitrary_ende_attn = config['decoder']['link']
        ...
        self.layers.extend([
            TransformerDecoderLayer(
                args, layer_idx=i, no_encoder_attn=no_encoder_attn, config=config)
            for i in range(self.super_layer_num)
        ])
        ...
        for i, layer in enumerate(self.layers):
            ...
            if encoder_out is not None:
                ...
                # concat one second last output layer
                    ...
                    encoder_out_feed = torch.cat([encoder_out['encoder_out'], encoder_out['encoder_out_all'][-2]],
                                                 dim=0)
                elif self.sample_arbitrary_ende_attn[i] == 2:
                    encoder_out_feed = torch.cat([encoder_out['encoder_out'], encoder_out['encoder_out_all'][-2],
                                                  encoder_out['encoder_out_all'][-3]], dim=0)
                else:
                ...
                # concat two more
                    ...
                    encoder_padding_mask_feed = torch.cat(
                        [encoder_out['encoder_padding_mask'], encoder_out['encoder_padding_mask'],
                         encoder_out['encoder_padding_mask']], dim=1)
                else:
                    raise NotImplementedError(
                        "arbitrary_ende_attn should in [-1, 1, 2]")
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, layer_idx, config=None):
        super().__init__()
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, layer_idx, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False, config=None):
        super().__init__()
        ...
        if config is None:
            ...
            self.super_embed_dim = config['decoder']['emb_dim']
            self.super_encoder_embed_dim = config['encoder']['emb_dim']
            ...
            self.super_ende_attention_heads_this_layer = config[
                'decoder']['en_de_attn_heads'][layer_idx]
        ...
        if no_encoder_attn:
            ...
            if self.fixed:
                self.encoder_attn_layer_norm = LayerNorm(self.super_embed_dim)
            else:
                self.encoder_attn_layer_norm = LayerNormSuper(
                    self.super_embed_dim)
    ...
    def set_sample_config(self, is_identity_layer, sample_embed_dim=None, sample_encoder_embed_dim=None,
                          sample_ffn_embed_dim_this_layer=None, sample_self_attention_heads_this_layer=None,
                          sample_ende_attention_heads_this_layer=None, sample_dropout=None,
                          sample_activation_dropout=None):
        ...
        self.self_attn.set_sample_config(sample_q_embed_dim=self.sample_embed_dim,
                                         sample_attention_heads=self.sample_self_attention_heads_this_layer)
        self.encoder_attn.set_sample_config(sample_q_embed_dim=self.sample_embed_dim,
